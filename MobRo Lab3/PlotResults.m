...
% of the file, here calcPhase, t, x, y, theta, P11...P33, U1, U2, Y
...
for i = 1 : length(t) 
    ...
    % Measurement: y coordinate of the magnet measured in Rm.
    Y = y2(i) ;
    
    % Homogeneous coordinates of the magnet in Rm for calculations.
    mMeasMagnet = [ y1(i) ; Y ; 1 ] ;
    ...
    C = [ sin(theta(i)) , -cos(theta(i))   -sin(theta(i))*(oRealMagnet(2)-y(i))-cos(theta(i))*(oRealMagnet(1)-x(i)) ] ;
    ...
    for neighborIndex = 1 : 4
        ...
        YhatNeighbor = YhatNeighbor(2) ;

        Cneighbor = [ sin(theta(i)) , -cos(theta(i)) , -sin(theta(i))*(oPneighbor(2)-y(i))-cos(theta(i))*(oPneighbor(1)-x(i)) ] ;

        innovNeighbor = Y - YhatNeighbor ; 
