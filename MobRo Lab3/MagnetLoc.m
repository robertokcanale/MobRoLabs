...
X = [ 0, 0,0*pi/180 ].' ;    % Set this accheording to robot initial position.
...
for i = 2 : nbLoops 
    ...
    A = [ 1, 0, -U(1)*sin(X(3));
        0, 1, U(1)*cos(X(3));
        0, 0, 1 ];  %respectively, x, y, theta
    B = [ cos(X(3)), 0; 
        sin(X(3)) 0;
        0, 1 ] ;  %respectively, u1, u2
    ...
    for measNumber = 1 : length(measures) 
        ...
        % --> Y = [ cos( X(3) ) * ( oRealMagnet(1) - X(1) ) + sin( X(3) ) * (oRealMagnet(2) - X(2) );
        %           cos( X(3) ) * ( oRealMagnet(2) - X(2) ) - sin( X(3) ) * (oRealMagnet(1) - X(1) ) ] ;
        ...
        C = [ sin( X(3) ), -cos( X(3) ), -sin( X(3) ) * ( oRealMagnet(2) - X(2) ) - cos( X(3) ) * (oRealMagnet(1) - X(1) )] ;
                      
        innov = Y - Yhat ;   
        dMaha = sqrt( innov.' * inv( C*P*C.' + Qgamma) * innov ) ; %Qgamma is the variance of Y, CPC is the variance of Yhat
        
        LogData( t , 'measurement' , X , P , [0;0] , [sensorPosAlongXm;Y] );
